(function () {
  ...
  function colgroupMarkup(columns, demand = {}) {
    const widths = NS.TableBlock.planColumnWidths(columns, demand);
    return `<colgroup>${widths.map(column => `<col data-table-column-width="${Render.esc(column.id)}" data-table-column-demand="${Render.esc(column.demand)}" style="width:${column.percent}%" />`).join('')}</colgroup>`;
  }
  ...
  function tableMarkup(item, showPrices) {
    ...
    const priceStyle = hasPrice && showPrices ? (block.priceStyle || 'standard') : 'standard';
    const commercialClass = priceStyle !== 'standard' ? ' commercial-prices' : '';
    return `<section class="catalog-table-block density-${Render.esc(block.density)} price-style-${Render.esc(priceStyle)}${continuation ? ' is-continuation' : ''}${commercialClass}" data-table-block-id="${Render.esc(block.id)}" data-row-span="${item.rowSpan}" data-fragment-start="${item.fragmentStart}" data-fragment-end="${item.fragmentEnd}" data-fragment-total="${item.fragmentTotal}" data-price-style="${Render.esc(priceStyle)}" style="${placement}">
      ...
      <div class="catalog-table-wrap"><table>
        ${colgroupMarkup(columns, item.columnDemand || {})}
        ...
        <tbody>${item.rows.map(row => `<tr data-table-row-id="${Render.esc(row.rowId)}" data-product-id="${Render.esc(row.productId || '')}">${columns.map(columnId => cellMarkup(columnId, row)).join('')}</tr>`).join('')}</tbody>
      </table></div>
    </section>`;
  }
