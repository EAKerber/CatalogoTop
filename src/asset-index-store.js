(function () {
  ...
  const { IndexedCache, AssetIndexSnapshot, ProductStore } = NS;
  if (!IndexedCache || !AssetIndexSnapshot || !ProductStore) return;
  ...
  async function putSnapshot(candidate) {
    ...
    const outgoing = AssetIndexSnapshot.forWrite({ revision, writeId, folders: candidate.folders, assets: candidate.assets });
  ...
  async function publishCandidate(candidate) {
    ...
    snapshot = candidate;
    pendingWrite = true;
    await IndexedCache.setAssetIndexSnapshot?.({ ...candidate, pendingWrite: true });
    notify({ type: 'asset-index-local-change' });
    try {
      if (!await ensureSession()) return false;
      let remote;
      try { remote = await putSnapshot(candidate); }
      catch (error) {
      ...
      if (error.code === 'revision_conflict') {
        conflict = true;
        await IndexedCache.setAssetIndexSnapshot?.({ ...candidate, pendingWrite: true, conflict: true });
  ...
  async function setLabel(assetRef, label, metadata = {}) {
    ...
    assets.push(next);
    const candidate = AssetIndexSnapshot.forWrite({ revision, folders: snapshot.folders, assets });
