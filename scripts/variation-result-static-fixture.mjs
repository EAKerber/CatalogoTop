...
const checks = [
  ...
  ['pacote inteiro valida antes de prepare/upload', controls.indexOf('validatePackage') < controls.indexOf('prepareValidated') && controls.indexOf('prepareValidated') < controls.indexOf('uploadWithSession')],
  ...
  ['catálogo é revalidado após upload e antes do commit', controls.indexOf('uploadWithSession') < controls.indexOf('result_request_changed_during_import') && controls.indexOf('result_request_changed_during_import') < controls.indexOf('commitUploaded')],
