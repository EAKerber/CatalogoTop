...
const checks = [
  ...
  ['CatalogDocument consome CatalogOrder e segue autoridade mista única', documentModel.includes('NS.CatalogOrder?.effectiveProducts') && documentModel.includes('function resolveBlocks') && documentModel.includes("type: 'collection'") && documentModel.includes("type: 'table-fragment'") && documentModel.includes('orderedIds = selected.map')],
