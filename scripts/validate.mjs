...
const requiredFiles = [
  ...
  'collection-block.css','table-block.css','product-actions.css','product-library.css','commercial-presentation.css',
  ...
  'src/collection-controls.js','src/table-controls.js','src/product-actions.js','src/block-overlap-guard.js',
  'src/indexed-cache.js','src/asset-client.js','src/product-store.js','src/importer.js','src/templates.js','src/icons.js',
  'src/form-steps.js','src/mobile-workspace.js','src/preview-zoom.js','src/product-details.js','src/product-library.js','src/cadastro-surface.js',
...
const productLibrary = files['src/product-library.js'];
const cadastroSurface = files['src/cadastro-surface.js'];
const overlapGuard = files['src/block-overlap-guard.js'];
const printJs = files['src/print.js'];
const commercialCss = files['commercial-presentation.css'];
const groupingCss = files['grouping-controls.css'];
const logo = files['assets/logo-top-mobili.svg'];
const checks = [
  ['shell R1 possui Cadastro, Catálogo e Biblioteca sem Templates top-level', ['products','catalog','library'].every(tab => html.includes(`data-tab="${tab}"`)) && !html.includes('data-tab="templates"') && !html.includes('id="templates"')],
  ['shell não mantém botões do modo Agrupar aposentado', !html.includes('btnEnterGrouping') && !html.includes('btnCancelGrouping')],
  ['runtime R1 não carrega category-browser/delete UI legados', html.includes('src/cadastro-surface.js') && html.includes('src/product-library.js') && !html.includes('category-browser.css') && !html.includes('src/category-browser.js') && !html.includes('src/product-delete-ui.js')],
  ['Cadastro final não depende de shim/filesystem V1', cadastroSurface.includes('ProductSnapshot.assignPathProduct') && cadastroSurface.includes('ProductQuery.query') && cadastroSurface.includes('ProductDomain.cloneAsNewProduct') && !cadastroSurface.includes('data-r1d-legacy-product-list-compat') && !html.includes('data-r1d-legacy-product-list-compat') && !html.includes('id="categoryFolders"') && !html.includes('id="productRows"')],
  ...
  ['estado local migra para schema v9 com folders e template versionado', core.includes('SCHEMA_VERSION = 9') && core.includes('templateVersion: 1') && core.includes('folders: organization.folders || []') && core.includes("folderId: String(product.folderId || '').trim()") && core.includes('imageGallery: normalizeImageGallery') && core.includes('order: []') && core.includes('blocks: []') && html.indexOf('src/folder-tree.js') < html.indexOf('src/product-folder-migration.js') && html.indexOf('src/product-folder-migration.js') < html.indexOf('src/product-snapshot.js') && html.indexOf('src/product-snapshot.js') < html.indexOf('src/core.js')],
  ['Composition preserva order/blocks e contratos de imagem sem painel bulk legado', composition.includes('order: uniqueIds(source.order)') && composition.includes('blocks: normalizeBlocks(source.blocks)') && composition.includes('imageFrames: normalizeImageFrames(source.imageFrames)') && composition.includes('imageSelections: NS.ImageVariants.normalizeImageSelections') && composition.includes('imageVariants: NS.ImageVariants.normalizeCatalogImageVariants') && composition.includes('resolveImage') && !composition.includes('setupBulkControls') && !composition.includes('bulkPresentationControls')],
  ...
  ['Table é modelo fragmentável e adapta colunas por conteúdo', table.includes("type: 'table'") && table.includes('fragmentTable') && table.includes('capacityForUnit') && table.includes('columnDemand') && table.includes('planColumnWidths') && table.includes('elasticBounds') && table.includes('effectiveDemand') && !table.includes('Composition.normalizePresentation =')],
  ...
  ['apresentação comercial agrupada tem uma única autoridade documental', commercialCss.includes('.catalog-collection-item.price-style-label') && commercialCss.includes('.catalog-table-block.price-style-block') && !commercialCss.includes('.catalog-table-block.commercial-prices') && !commercialCss.includes('.inspector-commercial-toggle') && !groupingCss.includes('.catalog-collection-item.price-style-label') && !groupingCss.includes('.catalog-table-block.price-style-block')],
  ['exclusão destrutiva pertence à Biblioteca e reutiliza operação de domínio', !app.includes('data-delete-product-direct') && !app.includes('deleteProductFromUi') && productLibrary.includes('ProductActions.deleteProducts') && files['src/product-actions.js'].includes('cleanupDraftForDeletedProduct') && files['src/product-actions.js'].includes('presentation.order')],
  ['overlap guard não observa DOM', !overlapGuard.includes('MutationObserver')],
  ['print continua isolado com CSS documental completo', printJs.includes("querySelectorAll('.catalog-page')") && printJs.includes('collection-block.css') && printJs.includes('table-block.css') && printJs.includes('commercial-presentation.css') && !printJs.includes("href: 'grouping-controls.css'")],
  ...
  ['chrome do inspector é editor-only', files['contextual-inspector.css'].includes('#catalogPreview .editor-selected') && groupingCss.includes('@media print') && groupingCss.includes('.editor-multi-selected')],
  ...
  ['compositor segue container-aware sem painel bulk legado', files['composer-layout.css'].includes('container-type: inline-size') && !files['composer-layout.css'].includes('bulk-presentation-controls') && !groupingCss.includes('.selectable-products.is-grouping') && !files['composer-layout.css'].includes('btnEnterGrouping')],
  ['templates técnico/compacto/showcase permanecem como opções do Catálogo', ['technical','compact','showcase'].every(id => files['src/templates.js'].includes(`id: '${id}'`)) && html.includes('id="catalogTemplate"')],
  ['importador continua exigindo código/descrição e não inventa folderId V2', files['src/importer.js'].includes('Código e descrição são obrigatórios.') && files['src/importer.js'].includes('delete normalized.folderId')],
