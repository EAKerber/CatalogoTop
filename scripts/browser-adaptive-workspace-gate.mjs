...
function installFixture() {
  ...
  const products = Array.from({ length: 14 }, (_, index) => ({
    ...
    category: index < 9 ? 'CORREDIÇAS' : index < 13 ? 'DOBRADIÇAS' : 'SUPORTES',
    subcategory: '',
    price: `R$ ${10 + index},90`,
    status: 'Ativo',
    notes: '',
  ...
  NS.Core.setState({
    ...
    catalog: {
      ...
      presentation: NS.Composition.normalizePresentation({
        order: products.map(product => product.id), itemStyles: {}, imageFrames: {},
        blocks: [{ id: 'table-adaptive', type: 'table', memberIds: ['p1','p2','p3','p4','p5'], title: 'CORREDIÇAS', subtitle: '', rowSource: 'products', density: 'compact', columns: ['image','code','description','price'], priceStyle: 'label' }]
      })
    }
  }, { persist: false });
...
try {
  ...
  if (wide.previewOverflowY !== 'visible' || wide.panelMaxHeight !== 'none' || wide.listOverflowY !== 'auto') {
    throw new Error(`desktop largo ainda usa viewport vertical rígida: ${JSON.stringify(wide)}`);
  }
  ...
  const general = await page.evaluate(() => {
    ...
    const order = root.querySelector('.inspector-member-order');
    const fields = root.querySelector('[data-inspector-table]');
    return {
      mode: root.dataset.inspectorMode,
      overflowY: getComputedStyle(root).overflowY,
      orderVisible: order ? getComputedStyle(order).display !== 'none' : false,
      fieldsVisible: fields ? getComputedStyle(fields).display !== 'none' : false
    };
  });
  ...
  const ordering = await page.evaluate(() => {
    ...
    return {
      mode: root.dataset.inspectorMode,
      overflowY: getComputedStyle(root).overflowY,
      orderVisible: getComputedStyle(root.querySelector('.inspector-member-order')).display !== 'none',
      catalogOrderVisible: getComputedStyle(root.querySelector('.inspector-selection-order')).display !== 'none',
      fieldsVisible: getComputedStyle(root.querySelector('[data-inspector-table]')).display !== 'none'
    };
  });
  ...
  const scope = await page.evaluate(() => ({
    floaterHidden: document.querySelector('#editorOrderFloater')?.hidden,
    moved: window.CatalogoTop.GroupingControls.moveSelectionRelative(1),
    order: JSON.stringify(window.CatalogoTop.Core.getState().catalog.presentation.order),
    selected: JSON.stringify(window.CatalogoTop.ComposerSelection.get())
  }));
  await page.keyboard.press('Escape');
  const selectedAfterEscape = await page.evaluate(() => JSON.stringify(window.CatalogoTop.ComposerSelection.get()));
  if (!scope.floaterHidden || scope.moved || scope.order !== orderBefore || scope.selected !== selectedBefore || selectedAfterEscape !== selectedBefore) {
    throw new Error(`controles editoriais vazaram para fora de Catálogo: ${JSON.stringify({ scope, orderBefore, selectedBefore, selectedAfterEscape })}`);
  }
  ...
  const medium = await page.evaluate(() => ({
    toggleDisplay: getComputedStyle(document.querySelector('#catalogPanelToggle')).display,
    ...
    layoutWidth: document.querySelector('.selection-layout').getBoundingClientRect().width
  }));
  if (medium.toggleDisplay === 'none' || medium.panelPosition !== 'fixed' || medium.previewWidth < medium.layoutWidth * .92) throw new Error(`workspace médio não virou drawer sem comprimir A4: ${JSON.stringify(medium)}`);
  ...
  if (!drawer.open || !drawer.backdrop) throw new Error(`drawer médio não abriu corretamente: ${JSON.stringify(drawer)}`);
  ...
  const productMedium = await page.evaluate(() => {
    ...
    return {
      folderDisplay: getComputedStyle(folders).display,
      folderWrap: getComputedStyle(folders).flexWrap,
      folderOverflowX: getComputedStyle(folders).overflowX,
      columns: getComputedStyle(library).gridTemplateColumns,
      tableOverflow: table.scrollWidth - table.clientWidth
    };
  });
  if (productMedium.folderDisplay !== 'flex' || productMedium.folderWrap !== 'nowrap' || productMedium.folderOverflowX !== 'auto' || productMedium.tableOverflow > 3) {
    throw new Error(`Produtos médio chegou a overflow antes de mover pastas para cima: ${JSON.stringify(productMedium)}`);
  }
  ...
  const mobile = await page.evaluate(() => {
    ...
    const folderRow = document.querySelector('#categoryFolders .category-folder-row:not(.all)');
    const folder = folderRow?.querySelector('.category-folder');
    const trash = folderRow?.querySelector('.category-delete-button');
    ...
    return {
      ...
      folderGap: folderRect && trashRect ? Math.abs(folderRect.right - trashRect.left) : 999,
      libraryAlign: getComputedStyle(document.querySelector('.product-library')).alignContent
    };
  });
  if (mobile.rowHeight < 88 || mobile.thumbWidth < 46 || mobile.thumbDisplay === 'none' || mobile.clamp !== '3' || mobile.folderGap > 1 || mobile.libraryAlign !== 'start') {
    throw new Error(`biblioteca mobile não corresponde ao contrato visual: ${JSON.stringify(mobile)}`);
  }
  ...
  const settingsPosition = await page.evaluate(() => {
    const headerBottom = document.querySelector('.app-shell-header').getBoundingClientRect().bottom;
    const filterTop = document.querySelector('.selection-toolbar').getBoundingClientRect().top;
    const root = document.querySelector('#contextualInspector');
    return { headerBottom, filterTop, mode: root.dataset.inspectorMode, returning: document.querySelector('[data-editor-settings]').classList.contains('is-returning') };
  });
  if (Math.abs(settingsPosition.filterTop - settingsPosition.headerBottom) > 32 || settingsPosition.mode !== 'general' || !settingsPosition.returning) {
    throw new Error(`⚙ não posicionou filtro/configuração como esperado: ${JSON.stringify(settingsPosition)}`);
  }

  await page.click('[data-editor-settings]');
  await page.waitForTimeout(450);
  const targetPosition = await page.evaluate(() => {
    const target = window.CatalogoTop.ComposerSelection.get();
    const node = window.CatalogoTop.ContextualInspector.previewNodeForTarget(target);
    const rect = node?.getBoundingClientRect();
    return { top: rect?.top, bottom: rect?.bottom, viewport: innerHeight, returning: document.querySelector('[data-editor-settings]').classList.contains('is-returning') };
  });
  if (targetPosition.top == null || targetPosition.bottom < 0 || targetPosition.top > targetPosition.viewport || targetPosition.returning) {
    throw new Error(`segundo ⚙ não voltou ao target selecionado: ${JSON.stringify(targetPosition)}`);
  }
