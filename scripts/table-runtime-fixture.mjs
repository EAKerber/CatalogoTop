...
['UI Table é creation-only e usa seleção estrutural externa',files['src/table-controls.js'].includes('createTable')&&files['src/table-controls.js'].includes('GroupingControls?.candidateIds')&&!files['src/table-controls.js'].includes('MutationObserver')&&!files['src/table-controls.js'].includes('data-block-member-delta')],
['GroupingControls é efêmero e não muta domínio',files['src/grouping-controls.js'].includes('const markedIds = new Set()')&&!files['src/grouping-controls.js'].includes('Core.mutate')],
