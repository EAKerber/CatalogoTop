...
A linha V2 concluiu R1, R2, R3, **R4 — Constrained Template System 2.0**, **R5 — Editorial Vocabulary 2.0**, **R6a — Structural Preflight Foundation** e **R6b — Rendered Description Truncation**.

Authority funcional atual:

- `v2@f589053dcee8aac7b37d417b3036cd92513f24cc` — R6b funcional promovido por PR #73;
...
R6b está completo. **Não existe R6c selecionado.** O próximo passo é biópsia dos sinais restantes, cada um segundo sua authority/lifecycle real.

## Ordem de leitura

1. `docs/v2/START-HERE.md` — bootstrap e fronteiras atuais;
2. `docs/v2/R6B-CLOSEOUT.md` — promotion, gates, aprendizado do fixture e estado pós-R6b;
3. `docs/v2/R6B-RENDERED-DESCRIPTION-TRUNCATION-INTENT.md` — contrato final de R6b;
4. `docs/v2/R6A-CLOSEOUT.md` e `R6A-STRUCTURAL-PREFLIGHT-FOUNDATION-INTENT.md` — fundação estrutural do Preflight;
5. `docs/v2/ROADMAP.md` — milestones e próximo ponto de decisão;
6. `docs/v2/R5-CLOSEOUT.md` e `CI-H1-ASSET-INDEX-WRITE-SETTLEMENT.md` — fechamento editorial e hardening de CI;
...
## Próximo ponto de decisão — biópsia pós-R6b

Não nomear R6c por sequência numérica.

Candidatos ainda plausíveis, mas não equivalentes:

- **imagem realmente quebrada após load** — lifecycle assíncrono; precisa definir quando o preview é confiável e distinguir placeholder intencional de URL que falhou;
- **collision/overflow** — exige geometria real e uma política de estabilidade; não copiar heurísticas de browser gate diretamente para runtime;
- **logical vs physical pages** — pode pertencer ao isolated print/release gate, não ao live Preflight;
- **Table factual visibility** — hoje não possui o sinal explícito de TextFit usado por R6b.

Para qualquer próximo recorte, preferir um sinal explícito já produzido pelo sistema. Se a única forma de detectar o problema for criar um scanner genérico de geometria ou uma segunda materialização, parar e replanejar antes de implementar.
